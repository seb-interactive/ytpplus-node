...
class {
    ...
    async getVideoLength(video,callback){
        return ffprobe(video,{path: ffprobeStatic.path})
     ...
     */
    snipVideo(video, startTime, endTime, output,resolution){
        var command = ffmpegStatic.path
            + " -i " + video
    ...
    copyVideo(video, output,resolution){
        var command1 = ffmpegStatic.path
            + " -i " + video
            + " -ar 44100"
            + " -ac 1"
    ...
    concatenateVideo(count, out) {
        try {
            ...
            return this.spawn(ffmpegStatic.path+command1,{})
            //cmdLine = CommandLine.parse(command2);
            //executor = new DefaultExecutor();
            //exitValue = executor.execute(cmdLine);
            
            //temp.delete();
        } catch (ex) {console.error(ex);rej(ex.toString())}
    }
}
