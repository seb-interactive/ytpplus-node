const pawn = require('child_process').execSync,
    imagemagick = require("imagemagick-cli"),
    fs = require("fs"),
    ...
    class {
        ...
        async configurateAndGo(options) {
            return new Promise(async (resolve,reject) => {
                ...
                this.OUTPUT_FILE = options.OUTPUT_FILE != undefined ? options.OUTPUT_FILE : "./done.mp4"
                this.MAX_CLIPS = options.MAX_CLIPS != undefined ? options.MAX_CLIPS : 5
                this.toolBox.setTemp("./temp/" + "job_" + (options.date != undefined ? options.date : new Date()).getTime() + "/");
                fs.mkdirSync(this.toolBox.TEMP)
                this.toolBox.SOURCES = options.sources != undefined ? options.sources : "./sources/";
                this.toolBox.SOUNDS = options.sounds != undefined ? options.sounds : "./sounds/";
                this.toolBox.MUSIC = options.music != undefined ? options.music : "./music/";
                this.toolBox.RESOURCES = "./resources/";
                this.sourceList = options.sourceList != undefined ? options.sourceList : ["./intro2.mp4"];

                this.MAX_STREAM_DURATION = options.MAX_STREAM_DURATION != undefined ? options.MAX_STREAM_DURATION : 0.4;
                this.MIN_STREAM_DURATION = options.MIN_STREAM_DURATION != undefined ? options.MIN_STREAM_DURATION : 0.4;
                ...
                //NEW IMPLEMENTATIONs
                this.effectRange = options.effectRange != undefined ? [(options.effectRange[0] != undefined ? options.effectRange[0] : 0),((options.effectRange[1] != undefined ? options.effectRange[1] : 11)+1)] : [0,12]
                ...
                try {
                    ...
                    for(var i = 0; i < oh.MAX_CLIPS; i++) {
                        oh.doneCount = i / oh.MAX_CLIPS;
                        var sourceToPick = oh.sourceList[oh._randomIntFromInterval(0, oh.sourceList.length - 1)];
                        ...
                        if(random == 15 && oh.insertTransitionClips == true) {
                            ...
                            donedoingthat = await oh.toolBox.snipVideo(sourceToPick, startOfClip, endOfClip, oh.toolBox.TEMP + "video" + i,oh.resolution);
                        }
                        if(donedoingthat) {
                    ...
                    setTimeout(async function() {
                        ...
                        if(isok) {
                            var okbuddy
                            okbuddy = await oh.toolBox.concatenateVideo(max, oh.OUTPUT_FILE);
                            while(!okbuddy) {
                                setTimeout(() => {}, 500)
                            }
